"""
Linear Quadratic Regulator (LQR) Optimal Control Benchmark
===========================================================
Benchmark for the LQR problem via the Hamilton-Jacobi-Bellman equation.

Problem: Minimize J = ∫₀ᵀ ½|u|² dt + ½|x(T)|²
Dynamics: ẋ = u  (free particle)
...
Verification: ∂φ/∂τ = -½|x|²/(1+τ)², ∇φ = x/(1+τ), ½|∇φ|² = ½|x|²/(1+τ)²  ✓

Test Cases: 1D through 10D, both grid and random points.
...
# PROBLEM DEFINITION
# ==========================================
YOUR_PROBLEM_NAME = "LQR_Optimal_Control"


def hamiltonian(p, **params):
    """
    Hamiltonian for LQR: H(p) = ½|p|²  (optimal control cost).
...
def initial_condition(spatial_points, **params):
    """
    Terminal cost at τ=0 (corresponds to time t=T).
    φ(x, 0) = ½|x|²
...
def reference_solution(spatial_points, time_points, **params):
    """
    Analytical value function: φ(x, τ) = ½|x|² / (1 + τ).
    """
    if spatial_points.ndim == 1:
        return _reference_1d(spatial_points, time_points)
    elif spatial_points.ndim == 3:
        return _reference_2d_grid(spatial_points, time_points)
    else:
        return _reference_points(spatial_points, time_points)
...
def _reference_2d_grid(spatial_points, t):
    # spatial_points: (nx, ny, 2), return (nx, ny, nt)
    nx, ny, _ = spatial_points.shape
    x2 = np.sum(spatial_points**2, axis=-1)  # (nx, ny)
...
def _reference_points(spatial_points, t):
    # spatial_points: (n_pts, dim), return (n_pts, nt)
    x2 = np.sum(spatial_points**2, axis=-1)  # (n_pts,)
    u = np.zeros((len(x2), len(t)))
    for k, tau in enumerate(t):
        u[:, k] = 0.5 * x2 / (1.0 + tau)
    return u
...
def create_visualizations(results):
    """Visualize LQR benchmark results."""
    ...
    for i, result in enumerate(successful):
        ...
        if dim == 1:
            plot_1d_results(result, filename=f'lqr_1d_case{i+1}.png')
        elif dim == 2:
            plot_2d_results(result, filename=f'lqr_2d_case{i+1}.png')

    plot_scaling_analysis(successful, filename='lqr_scaling_analysis.png')
...
def get_problem_specific_info():
    return {
        ...
        "category": "OC",
        "short_description": "HJB equation for the free-particle LQR problem with quadratic terminal cost",
        "equation": "∂φ/∂τ + ½|∇φ|² = 0",
        "initial_condition": "φ(x, 0) = ½|x|²  (quadratic terminal cost)",
        "background": "Classical LQR with dynamics ẋ=u, cost ∫½|u|²dt + ½|x(T)|²",
        "applications": ["Optimal control", "Robot trajectory planning", "HJB solver validation"],
    }
...
def get_case_description(dimension, params, problem_info):
    ...
    return f"{dimension}D LQR value function ({pt})"
...
def get_testing_aspects(dimensions, parameter_variations, problem_info):
    ...
    aspects["analytical_reference"] = "Exact value function φ(x,τ) = ½|x|²/(1+τ)"
    aspects["quadratic_ic"] = "Tests handling of quadratic initial conditions"
    return aspects
